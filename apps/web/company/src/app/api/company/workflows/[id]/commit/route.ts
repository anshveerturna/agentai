...
async function buildAuthHeaders(req: NextRequest) {
  ...
  if (!headers.has('authorization')) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      console.warn('Commit proxy (DEV): injecting dummy Authorization')
    } else {
...
export async function POST(req: NextRequest, ctx: any) {
  try {
    ...
    const res = await fetch(`${apiBase}/workflows/${id}/commit`, { method: 'POST', headers, body, credentials: 'include', cache: 'no-store' })

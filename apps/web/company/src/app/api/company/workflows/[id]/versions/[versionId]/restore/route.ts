...
async function buildAuthHeaders(req: NextRequest) {
  ...
  if (!headers.has('authorization')) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      console.warn('Version restore proxy (DEV): injecting dummy Authorization')
    } else {
...
export async function POST(req: NextRequest, ctx: any) {
  ...
  const { id, versionId } = await ctx.params
    const res = await fetch(`${apiBase}/workflows/${id}/versions/${versionId}/restore`, { method: 'POST', headers, credentials: 'include', cache: 'no-store' })

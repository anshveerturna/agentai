...
async function buildAuthHeaders(req: NextRequest) {
  ...
  if (!headers.has('authorization')) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      console.warn('Versions proxy (DEV): injecting dummy Authorization')
    } else {
...
export async function GET(req: NextRequest, ctx: any) {
  ...
  const { id } = await ctx.params
    const res = await fetch(`${apiBase}/workflows/${id}/versions`, { headers, credentials: 'include', cache: 'no-store' })
...
export async function POST(req: NextRequest, ctx: any) {
  ...
  const { id } = await ctx.params
    const res = await fetch(`${apiBase}/workflows/${id}/versions`, { method: 'POST', headers, body, credentials: 'include', cache: 'no-store' })
    const text = await res.text()
    return new NextResponse(text, { status: res.status, headers: { 'content-type': res.headers.get('content-type') || 'application/json' } })
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : String(e)
